"""
Database utilities for working with UKOAA P1/90 navigation data

Database format: SQLite and SpatialLite
=======================================

This database is a SQLite_ database and utilizes the SpatiaLite_ libraries
to manage geometries and projections. All geometry data are stored in 
SpatialLite geometry columns, and plain text access to coordinates are
generally provided by standard views.  Thus, editing geometry requires
modifying the geometry fields in the data tables themselves.

Development notes
=================

This module should only include methods for inserting and retrieving data
from the database. Other utilities (re-tooling geom, etc.) will be
implemented in a higher-level P190() class.  The purpose of this is to make
a class for simple i/o of P190 data, without all the extra tools
"""
import os
import warnings
...
class P190Database(Connection):
    ...
    def __init__(self, database=':memory:',
        ...
        Connection.__init__(self, database, params_table='p190_params',
                spatial=True)

        output_srid = kwargs.pop('output_srid', input_srid)
    ...
    def _create_table_coord(self):
        ...
        sql = """CREATE TABLE IF NOT EXISTS '{:}' (
            ...
            spare TEXT,
            spare2 TEXT,
            PRIMARY KEY (line, point, record_id),
    ...
    def _create_table_rec_pt(self):
        ...
        sql = """CREATE TABLE IF NOT EXISTS '{self.REC_PT_TABLE}' (
            ...
            cable_depth REAL,
            PRIMARY KEY (line, point, chan, cable_id));
            """.format(**locals())
        self.execute(sql)
    ...
    def _create_view_rec_line(self):
        ...
        sql = """CREATE VIEW IF NOT EXISTS '{:}' AS SELECT
            line, point, count(chan) as nchan, MakeLine(rec_pt) as rec_line
             FROM '{:}' GROUP BY line, point ORDER BY chan
    ...
    def _create_view_src_rec(self):
        ...
        sql = """CREATE VIEW IF NOT EXISTS '{:}' AS SELECT
            ...
            r.line as line, r.point as point, r.cable_id as cable_id,
            r.chan as chan, s.geom as src_pt, r.rec_pt as rec_pt,
            Line_Interpolate_Point(MakeLine(s.geom, r.rec_pt), 0.5) as
            mid_pt FROM '{:}' as r INNER JOIN '{:}' as s ON r.point=s.point
            WHERE s.record_id='S'
    ...
    def _parse_coord(self, line):
        ...
        # line, point, record_id, vessel_id,
        # source_id, tailbuoy_id, water_depth_or_elev, 
        # day_of_year, time, spare, spare2,
        # geom
        return [line[1: 13], line[19: 25], line[0: 1], line[16: 17],
                line[17: 18], line[18: 19], line[64: 70],
                line[70: 73], line[73: 79], line[13: 16], line[79: 80],
